...
load_program() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^[[:space:]]*([0-9]+)[[:space:]]+(.*)$ ]]; then
            ...
            # Split multiple statements by colon (GW-BASIC compatibility)
            # But preserve colons in strings and comments
            local stmts=()
            local upper_stmt="${statement^^}"
            
            # If line is a REM or starts with ', don't split it
            ...
            # Store statements - use original line number for first, then line.subindex for rest
            # We need to keep original line numbers for GOTO/GOSUB compatibility
            # Use format: line number gets stored directly, additional statements use line_num.1, line_num.2, etc.
            # Since bash associative arrays handle string keys, we can use "10", "10.1", "10.2"
            if [[ ${#stmts[@]} -gt 0 ]]; then
...
find_next_line() {
    ...
    # Linear search for next line (handles fractional line numbers)
    local next=""
    for line in "${line_numbers[@]}"; do
        # Use awk for proper decimal comparison
        if awk -v curr="$current" -v check="$line" 'BEGIN { exit !(check > curr) }'; then
            next="$line"
            break
        fi
    done
