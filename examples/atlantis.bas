5 RANDOMIZE TIMER: SCREEN 2: GOSUB 5000
10 DIM cm$(31), ro$(32), rn(32, 4), ob$(33, 8), p$(35): CLS
20 ob$(1, 1) = "immovable door": ob$(2, 1) = "a delicious looking malt": ob$(2, 2) = "* platinum spoon"
30 ob$(5, 1) = "sign": ob$(6, 1) = "tombstone": ob$(7, 1) = "motocycle": ob$(8, 1) = "woman": ob$(10, 1) = "folded piece of paper"
40 ob$(12, 1) = "safe": ob$(16, 1) = "manhole": ob$(16, 2) = "dirtpile": ob$(17, 1) = "gas container": ob$(17, 2) = "dirtpile": ob$(21, 1) = "a large pool"
50 ob$(22, 1) = "duct tape": ob$(24, 1) = "sign": ob$(24, 2) = "a very large machine": ob$(25, 1) = "prybar": ob$(26, 1) = "* diamonds"
60 ob$(27, 1) = "scientific book": ob$(28, 1) = "a large cabinet": ob$(30, 1) = "sign": ob$(31, 1) = "crate"
70 ob$(32, 1) = "dying plant": ob$(26, 2) = "a very large machine": ob$(18, 1) = "ladder": FOR x = 1 TO 31: READ cm$(x): NEXT x
80 FOR d = 1 TO 32: FOR x = 1 TO 4
90  READ rn(d, x): NEXT x: NEXT d
95  FOR f = 1 TO 32: READ ro$(f): NEXT f
96 rn = 1
100 LOCATE 1, 1: FOR x = 1 TO 12: PRINT "                                                                        "; : NEXT x: LOCATE 1, 1
101 PRINT "you are in a "; ro$(rn)
105 PRINT "Exits are: ";
110 FOR x = 1 TO 4: IF rn(rn, x) <> 0 THEN PRINT LEFT$(cm$(x), 1); " ";
111 NEXT x: PRINT
115 PRINT "Here you see: ";
116 obj = 0: FOR x = 1 TO 8
117 IF ob$(rn, x) <> "" THEN obj = 1
118 NEXT x
119 IF obj = 0 THEN PRINT "Nothing."
120 FOR x = 1 TO 8
121 IF ob$(rn, x) <> "" THEN PRINT ob$(rn, x); "    ";
122 NEXT x: PRINT
130 PRINT "------------------------------------------------------------------------"
131 PRINT "                                                                   "
132 LOCATE 12, 1
133 IF rn = 27 THEN GOSUB 3800
135 INPUT "command"; a$: PRINT "                                                                     ": PRINT "                                                                ": LOCATE 14, 1
   IF a$ = "quit" THEN END
...
158    _
' IF LEFT$(p$(i), 1) = "s" AND p$(i) <> "spa" AND p$(i) <> "say" AND p$(i) <> "sci" AND p$(i) <> "str" AND p$(i) <> "sho" AND p$(i) <> "spo" AND p$(i) <> "sco" AND p$(i) <> "sig" AND p$(i) <> "saf" AND p$(i) <> "sha" AND p$(i) <> "swi" THEN p$(i) = "sou"
159  IF p$(i + 1) = "tap" THEN p$(i + 1) = "duc"
160  IF p$(i + 1) = "boo" THEN p$(i + 1) = "sci"
161  IF p$(i + 1) = "pap" AND up = 0 THEN p$(i + 1) = "fol"
...
167  IF p$(i + 1) = "cab" AND bc = 0 THEN p$(i + 1) = "a l"
...
174 IF p$(i + 1) = "cab" AND bc = 1 THEN p$(i + 1) = "a b"
...
201 FOR i = 0 TO x1
202 FOR ds = 1 TO 31
205 IF p$(i) = cm$(ds) THEN ON ds GOSUB 1000, 1000, 1000, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2700, 2800, 2900, 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700
210 NEXT ds, i
220 GOTO 100
1000 FOR x = 1 TO 4
1010 IF cm$(x) = p$(i) AND rn(rn, x) <> 0 THEN 1030
1020 NEXT x: PRINT "that direction is not an exit!": FOR f = 1 TO 1050: NEXT f: RETURN
1030 nr = rn(rn, x): rn = nr: RETURN
1100 IF rn = 16 AND p$(i + 1) = "man" THEN GOTO 1140
1105 GOTO 1199
1140 FOR x = 0 TO 9
1145 IF LEFT$(inv$(x), 3) = "mot" THEN 1155
...
1300 REM get
1301 FOR x = 1 TO 8
1329 IF p$(i + 1) = LEFT$(ob$(rn, x), 3) THEN GOTO 1350
...
1366 IF p$(i + 1) = "* n" THEN GOTO 1383
...
1375 FOR x = 1 TO 8: IF p$(i + 1) = LEFT$(ob$(rn, x), 3) THEN GOTO 1377
1376 NEXT x: STOP
1377 FOR d = 0 TO 9: IF inv$(d) <> "" THEN 1380
1378 IF p$(i + 1) = LEFT$(ob$(rn, x), 3) THEN inv$(d) = ob$(rn, x)
1379 ob$(rn, x) = "": PRINT "taken!": RETURN
1380 NEXT d: PRINT "no room in inventory.": RETURN
1383 FOR d = 0 TO 9: IF inv$(d) = "* cross" THEN GOTO 1375
1384 NEXT d: IF rn = 6 THEN PRINT "the body springs to life and kills you!": END
1385 GOTO 1375
1400 FOR x = 0 TO 9
1405 IF LEFT$(inv$(x), 3) = p$(i + 1) THEN GOTO 1415
1410 NEXT x: PRINT "you don't have it!": RETURN
1415 FOR d = 1 TO 8
1420 IF ob$(rn, d) = "" THEN 1430
1425 NEXT d: PRINT "no room to drop it!": RETURN
1430 ob$(rn, d) = inv$(x): inv$(x) = "": PRINT "dropped.": RETURN
1500 CLS : PRINT "your inventory: "
1505 FOR x = 0 TO 9: PRINT inv$(x): NEXT x
1510 INPUT "press return to continue"; a$: RETURN
1600 rh = RND * 6
1605 rh = INT(ABS(rh) + 1)
1610 ON rh GOTO 1620, 1630, 1640, 1650, 1660, 1670
1615 GOTO 1600
...
1710 IF rn = 32 AND p$(i + 1) = "dyi" AND wp = 0 THEN PRINT "the plant is murmuring water.....water.": RETURN
...
2000 IF rn = 6 AND db = 0 THEN GOTO 2040
2001 IF rn = 6 AND db = 1 THEN PRINT "you can't dig anymore!": RETURN
2003 GOTO 2040
2040 FOR x = 0 TO 9
2045 IF LEFT$(inv$(x), 3) = "sho" THEN 2055
...
2062 IF ob$(rn, x) = "" THEN 2070
2064 NEXT x: PRINT "remove an item from room and dig again!": RETURN
2070 ob$(rn, x) = "body": db = 1: RETURN
2099 PRINT "you find nothing special.": RETURN
2100 IF rn = 16 AND p$(i + 1) = "dir" THEN GOTO 2140
2101 IF rn = 17 AND p$(i + 1) = "dir" THEN GOTO 2140
2103 PRINT "you can't jump that!": RETURN
2140 FOR x = 0 TO 9
2145 IF LEFT$(inv$(x), 3) = "mot" THEN 2155
...
2205 IF LEFT$(ob$(5, x), 1) = "*" THEN ts = ts + 1
...
2405 IF p$(i + 1) = "pap" AND up = 1 THEN 2440
2406 IF p$(i + 1) = "fol" AND up = 0 THEN 2460
2407 IF p$(i + 1) = "sci" AND rs <> 1 THEN GOTO 2476
2408 IF p$(i + 1) = "sci" AND rs = 1 THEN GOTO 2476
2439 GOTO 2499
2440 FOR x = 0 TO 9
2445 IF LEFT$(inv$(x), 3) = "pap" THEN 2455
2450 NEXT x: PRINT "you aren't carrying the piece of paper!": RETURN
2455 PRINT "it's the combination to a safe!": cs = 1: RETURN
2460 FOR x = 0 TO 9
2465 IF LEFT$(inv$(x), 3) = "fol" THEN 2475
2470 NEXT x: PRINT "you aren't carrying the piece of paper!": RETURN
2475 PRINT "you have to unfold it first!": RETURN
2476 FOR x = 0 TO 9
2477 IF LEFT$(inv$(x), 3) = "sci" THEN 2485
...
2500 IF p$(i + 1) = "sco" THEN GOTO 2200
2599 PRINT "nothing happens!": RETURN
2600 IF p$(i + 1) = "fol" THEN GOTO 2640
2610 IF p$(i + 1) = "pap" THEN PRINT "you already unfolded that!": RETURN
2615 GOTO 2699
2640 FOR x = 0 TO 9
2645 IF LEFT$(inv$(x), 3) = "fol" THEN 2655
...
2710 GOTO 2799
2740 FOR x = 0 TO 9
2742 IF LEFT$(inv$(x), 3) = "gas" THEN 2755
2744 NEXT x: PRINT "you don't have the gas can.": RETURN
2755 FOR x = 1 TO 8
2756 IF ob$(rn, x) = "" AND fd = 0 THEN PRINT "you found something!": ob$(rn, x) = "* spanish doubloon": fd = 1: RETURN
2760 NEXT x: PRINT "you find nothing.": RETURN
2799 PRINT "nothing happens.": RETURN
2800 IF p$(i + 1) = "a d" THEN 2840
2810 GOTO 2899
2840 FOR x = 0 TO 9
2842 IF LEFT$(inv$(x), 3) = "a d" THEN 2846
...
2903 GOTO 2999
2940 FOR x = 0 TO 9
2942 IF LEFT$(inv$(x), 3) = "can" THEN GOTO 2944
2943 NEXT x
...
3200 IF rn = 28 AND p$(i + 1) = "a l" AND tg = 0 THEN 3240
3205 IF rn = 28 AND p$(i + 1) = "a l" AND tg = 1 THEN PRINT "the glass cabinet is already taped up!": RETURN
3210 GOTO 3240
3240 FOR x = 0 TO 9
3242 IF LEFT$(inv$(x), 3) = "duc" THEN GOTO 3246
3244 NEXT x: PRINT "you aren't carrying the duct tape!": RETURN
3246 IF rn = 28 THEN PRINT "you tape the glass cabinet with the duct tape.": tg = 1: RETURN
3299 PRINT "you can't tape that!": RETURN
3300 IF rn = 28 AND p$(i + 1) = "a l" AND tg = 1 THEN GOTO 3340
3310 IF rn = 28 AND p$(i + 1) = "a l" AND tg = 0 THEN PRINT "the flying glass cuts you to pieces!": END
3320 GOTO 3399
3340 FOR x = 1 TO 8
3342 IF LEFT$(ob$(rn, x), 3) = "a l" THEN 3350
3344 NEXT x: PRINT "you can't break it!": RETURN
3350 FOR d = 1 TO 8
3352 IF ob$(rn, d) = "" THEN 3354
3353 NEXT d: PRINT "remove an item from the room and try again.": RETURN
3354 ob$(rn, x) = "a broken cabinet": bc = 1: ob$(rn, d) = "* cross": PRINT "you find something!": RETURN
3399 PRINT "you can't break that!": RETURN
3400 IF p$(i + 1) = "wat" OR p$(i + 1) = "can" AND fc = 1 THEN 3405
3405 FOR x = 0 TO 9
3406 IF inv$(x) = "canteen" THEN GOTO 3440
3407 NEXT x: PRINT "you don't have the canteen!": RETURN
3440 IF rn = 32 AND fc = 1 THEN 3445
3441 IF fc = 1 AND rn <> 32 THEN GOTO 3499
3444 PRINT "the canteen is empty!": RETURN
3445  FOR x = 1 TO 8
3446 IF LEFT$(ob$(rn, x), 3) = "dyi" THEN ob$(rn, x) = "a plant"
3447 NEXT x: FOR x = 1 TO 8: IF ob$(rn, x) = "" AND fs = 0 THEN 3450
3448 NEXT x: PRINT "you already watered the plant.": RETURN
3450 PRINT "for your kindness you are rewarded!": fc = 0: ob$(rn, x) = "shovel": fs = 1: RETURN
3499 PRINT "the ground becomes wet.": fc = 0: RETURN
3500 IF rn = 32 AND p$(i + 1) = "dyi" THEN GOTO 3540
3510 IF rn = 32 AND p$(i + 1) = "a p" THEN PRINT "you already watered the dying plant!": RETURN
3540 FOR x = 0 TO 9
3541 IF inv$(x) = "canteen" THEN GOTO 3550
3542 NEXT x: PRINT "you don't have the canteen!": RETURN
3550 IF rn = 32 AND fc = 1 THEN GOTO 3445
3551 IF fc = 1 AND rn <> 32 THEN GOTO 3499
3552 PRINT "the canteen is empty!": RETURN
3600 IF rn = 12 AND cs = 1 AND fb = 0 AND p$(i + 1) = "saf" THEN GOTO 3640
3601 IF rn = 31 AND oc = 0 AND p$(i + 1) = "cra" THEN GOTO 3610
3602 IF rn = 31 AND oc = 1 AND p$(i + 1) = "cra" THEN PRINT "you opened the crate already!": RETURN
3603 IF rn = 12 AND cs = 0 AND p$(i + 1) = "saf" THEN PRINT "you don't know the combination!": RETURN
3604 IF rn = 12 AND fb = 1 AND p$(i + 1) = "saf" THEN PRINT "you already opened the safe!": RETURN
3605 PRINT "you can't open that.": RETURN
3610 FOR x = 0 TO 9
3611 IF LEFT$(inv$(x), 3) = "pry" THEN GOTO 3613
3612 NEXT x: PRINT "you don't have anything to pry with!": RETURN
3613 FOR x = 1 TO 8
3614 IF ob$(rn, x) = "" THEN 3616
3615 NEXT x: PRINT "remove an item from the room and try again.": RETURN
3616 ob$(rn, x) = "canteen": PRINT "you find something!": oc = 1: RETURN
3640 FOR x = 1 TO 8
3641 IF ob$(rn, x) = "" THEN 3643
3642 NEXT x: PRINT "remove an item from the room and try again.": RETURN
3643 ob$(rn, x) = "* moneybag": PRINT "you find something!": fb = 1: RETURN
...
3800 FOR x = 0 TO 9
3905 IF inv$(x) = "card" THEN RETURN
...
5000 CLS
5010 FOR x = 199 TO 0 STEP -2: FOR d = 1 TO 300: NEXT d
5020 LINE (0 + x, 0 + x)-(639 - x, 199 - x), , B
5025 NEXT x
5050 LOCATE 13, 26
5060 PRINT "Escape From Atlantis by Fly"
5065 LOCATE 14, 34: PRINT "Version 5.01"
5070 SLEEP 3
5080 RETURN